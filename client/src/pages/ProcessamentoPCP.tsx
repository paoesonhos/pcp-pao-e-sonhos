                  ...
                  return diasOrdenados.map(dia => (
                    <div key={`dia-${dia}`}>
                      ...
                      {porDia[dia].map((inter, interIdx) => {
                        // Ingredientes base da massa (apenas massa_base, secos e incluirPrePesagem = true)
                        const ingredientesBase = inter.ingredientes?.filter((ing: any) => ing.tipoComponente === 'massa_base' && ing.incluirPrePesagem !== false && ing.tipo === 'seco') || [];
                                ...
                                {produtosComAdicionais.map((produto, prodIdx) => (
                                  <div key={`prod-${prodIdx}`} className="bg-white border border-gray-200 rounded-lg p-4">
                                    ...
                                    <div className="space-y-1 text-sm text-gray-700 ml-4">
                                      {produto.insumos?.filter((ing: any) => ing.tipoComponente === 'ingrediente' && ing.incluirPrePesagem !== false && ing.tipo === 'seco').map((ing, idx) => (
                                        <div key={idx} className="flex justify-between">
                                          <span>- {ing.nomeComponente}:</span>
                                          <span className="font-mono">
